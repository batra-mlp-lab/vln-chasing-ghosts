...
class Unet5ContextualBneck(torch.nn.Module):
    def __init__(self, in_channels, out_channels, embedding_size, hc1=32, hb1=16, hc2=256, stride=2, split_embedding=False, norm="instance_norm"):
        ...
        if norm == "instance_norm":
            ...
            self.norm3 = nn.BatchNorm2d(hc1)
            self.norm4 = nn.BatchNorm2d(hc1)
            self.norm5 = nn.BatchNorm2d(hc1)
            # self.dnorm1 = nn.BatchNorm2d(in_channels * 4)
            self.dnorm2 = nn.BatchNorm2d(hc1)
            self.dnorm3 = nn.BatchNorm2d(hc1)
            self.dnorm4 = nn.BatchNorm2d(hc1)
            self.dnorm5 = nn.BatchNorm2d(hc2)

            self.fnorm1 = nn.BatchNorm2d(hb1)
            self.fnorm2 = nn.BatchNorm2d(hb1)
            self.fnorm3 = nn.BatchNorm2d(hb1)
            self.fnorm4 = nn.BatchNorm2d(hb1)
